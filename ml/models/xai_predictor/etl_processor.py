...
student_metrics = assessment_data.groupby(['id_student', 'code_module', 'code_presentation']).agg({
    'score': ['mean', 'std', 'count', 'min', 'max'],
    'date_submitted': 'max'
}).reset_index()

student_metrics.columns = ['id_student', 'code_module', 'code_presentation', 
...
df['high_credits'] = (df['studied_credits'] > df['studied_credits'].median()).astype(int)
...
# === SAVE PROCESSED DATA ===
output_cols = ['id_student', 'code_module', 'code_presentation', 
               'gender', 'region', 'highest_education', 'age_band',
               'final_result'] + feature_cols + ['is_at_risk']
...
print(f"Safe students (Pass/Distinction): {sum(output_df['is_at_risk']==0):,} ({sum(output_df['is_at_risk']==0)/len(output_df)*100:.1f}%)")
print(f"At-risk students (Fail/Withdrawn): {sum(output_df['is_at_risk']==1):,} ({sum(output_df['is_at_risk']==1)/len(output_df)*100:.1f}%)")
